...
/*
 * Ver.2.1.5のブラウザー回帰テストです。
 * リアルごほうび交換券、級別データ分離、5形式、旧データ移行、連打防止、
 * 戻る操作、復元、iPhone幅、A4印刷、全問題バンクをまとめて確認します。
 */
...
async function main() {
  ...
  /* 100%設定では交換券だけが出て、使用・設定削除後も券の表示情報が残ります。 */
  const rewardBefore = await page.evaluate(() => window.eval(`(() => {
    gachaDrawing = false;
    data.rewards.catalog = [{ id: "qa-reward", icon: "🥤", name: "コーラ1本", cost: 20, dropRate: 100 }];
    data.rewards.earned = [];
    data.gacha.tickets = 1;
    const ownedBefore = data.gacha.owned.length;
    drawGacha();
    return { ownedBefore, tickets: data.gacha.tickets };
  })()`));
  await page.waitForTimeout(1300);
  const realReward = await page.evaluate(() => window.eval(`(() => {
    const ticket = data.rewards.earned[0];
    const history = data.gacha.history.at(-1);
    const result = {
      tickets: data.gacha.tickets,
      owned: data.gacha.owned.length,
      earned: data.rewards.earned.length,
      source: ticket.source,
      historyType: history.type,
      modalBadge: document.getElementById("gachaRarity").textContent,
      modalName: document.getElementById("gachaName").textContent,
      total: rewardDropTotal()
    };
    ticket.used = true;
    ticket.usedAt = new Date().toISOString();
    ticket.rewardName = ticket.rewardName || "コーラ1本";
    ticket.rewardIcon = ticket.rewardIcon || "🥤";
    data.rewards.catalog = [];
    renderRewards();
    result.usedAt = Boolean(ticket.usedAt);
    result.nameAfterDelete = document.querySelector("#rewardWallet b").textContent;
    result.usedAfterDelete = document.querySelector("#rewardWallet small").textContent.includes("使用済み");
    return result;
  })()`));
  assert.equal(realReward.owned, rewardBefore.ownedBefore);
  delete realReward.owned;
  assert.deepEqual(realReward, {
    tickets: 0, earned: 1, source: "gacha", historyType: "reward",
    modalBadge: "交換券", modalName: "コーラ1本", total: 100,
    usedAt: true, nameAfterDelete: "コーラ1本", usedAfterDelete: true
  });
  await page.click("#gachaClose");
  ...
  assert.equal(versionFile.version, "2.1.5");
  ...
  const pdfPath = path.join(outputDirectory, "ver2_1_5-a4-8kanji.pdf");
